...
function getVenue(id) { return DB.venues.find(v => v.id === id); }
function getEvent(id) { return DB.events.find(e => e.id === id); }

function venueBadge(venueId) {
  const map = { 'rock-arena': ['badge-rock', 'RockArena'], 'techcon': ['badge-tech', 'TechCon'], 'summer-pavilion': ['badge-ibiza', 'Summer Beat'] };
  const [cls, label] = map[venueId] || ['badge-gray', venueId];
  return `<span class="badge ${cls}">${label}</span>`;
}

function typeBadge(type) {
  const map = { Festival: 'badge-purple', Concerto: 'badge-rock', 'DJ Set': 'badge-ibiza', Summit: 'badge-tech', Conferenza: 'badge-tech', Forum: 'badge-tech', Workshop: 'badge-green' };
  return `<span class="badge ${map[type] || 'badge-gray'}">${type}</span>`;
}

function statusBadge(s) {
  return s === 'confirmed' ? `<span class="badge badge-green">✓ Confermato</span>` : `<span class="badge badge-yellow">⏳ In attesa</span>`;
}
...
function fmtMoney(n) { return '€ ' + Number(n).toLocaleString('it-IT', { minimumFractionDigits: 2 }); }
...
function renderOverview() {
  ...
  document.getElementById('venue-cards').innerHTML = DB.venues.map(v => {
  ...
  const upcoming = [...DB.events].sort((a, b) => a.date.localeCompare(b.date)).slice(0, 8);
  document.getElementById('upcoming-count').textContent = upcoming.length + ' eventi';
  document.getElementById('overview-table').innerHTML = upcoming.map(e => `
    <tr>
...
function renderEventsTable() {
  ...
  const venueFilter = document.getElementById('events-venue-filter')?.value || '';
  ...
  data.sort((a, b) => {
    let av = a[eventsSortKey], bv = b[eventsSortKey];
  ...
  document.getElementById('events-table').innerHTML = data.length ? data.map(e => `
    <tr>
  ...
  `).join('') : `<tr><td colspan="8"><div class="empty-state"><div class="empty-state-icon">🔍</div><div class="empty-state-title">Nessun evento trovato</div><div class="empty-state-sub">Modifica i filtri o aggiungi un nuovo evento</div></div></td></tr>`;
}

function sortEvents(key) {
  if (eventsSortKey === key) eventsSortDir *= -1;
  else { eventsSortKey = key; eventsSortDir = 1; }
  document.querySelectorAll('[id^="sort-"]').forEach(el => el.textContent = '↕');
  const el = document.getElementById('sort-' + key);
  if (el) { el.textContent = eventsSortDir === 1 ? '↑' : '↓'; el.parentElement.classList.add('sorted'); }
  renderEventsTable();
}
...
function openEventModal(id = null) {
  editingEventId = id;
  document.getElementById('event-modal-title').textContent = id ? 'Modifica Evento' : 'Nuovo Evento';
  clearEventForm();
  if (id) {
    const e = getEvent(id);
    if (e) {
...
function clearEventForm() {
  ['evt-name', 'evt-venue', 'evt-type', 'evt-date', 'evt-capacity', 'evt-sold', 'evt-price'].forEach(id => {
    const el = document.getElementById(id);
    if (el) { el.value = ''; el.classList.remove('error'); }
  });
  ['err-evt-name', 'err-evt-venue', 'err-evt-type', 'err-evt-date', 'err-evt-capacity', 'err-evt-price'].forEach(id => {
    const el = document.getElementById(id);
...
function validateEvent() {
  let valid = true;
  const rules = [
    ['evt-name', 'err-evt-name', v => v.trim().length >= 3, 'Inserisci un nome di almeno 3 caratteri'],
    ['evt-venue', 'err-evt-venue', v => v !== '', 'Seleziona una struttura'],
    ['evt-type', 'err-evt-type', v => v !== '', 'Seleziona un tipo di evento'],
    ['evt-date', 'err-evt-date', v => v !== '', 'Inserisci una data valida'],
    ['evt-capacity', 'err-evt-capacity', v => parseInt(v) > 0, 'La capienza deve essere positiva'],
    ['evt-price', 'err-evt-price', v => parseFloat(v) > 0, 'Il prezzo deve essere positivo'],
  ];
  ...
  if (sold > cap) {
    document.getElementById('err-evt-capacity').textContent = 'I venduti non possono superare la capienza';
...
function renderBookingsTable() {
  ...
  const statusFilter = document.getElementById('bookings-status-filter')?.value || '';

  let data = DB.bookings.filter(b => {
    const evt = getEvent(b.eventId);
    const name = `${b.firstName} ${b.lastName} ${evt?.name || ''}`.toLowerCase();
    return name.includes(search) && (!statusFilter || b.status === statusFilter);
  });

  data.sort((a, b) => {
    let av = a[bookingsSortKey], bv = b[bookingsSortKey];
    if (typeof av === 'string') return av.localeCompare(bv) * bookingsSortDir;
    return (av - bv) * bookingsSortDir;
  });

  document.getElementById('bookings-count').textContent = data.length + ' prenotazioni';
  document.getElementById('bookings-table').innerHTML = data.length ? data.map(b => {
    const evt = getEvent(b.eventId);
    return `
      <tr>
  ...
  }).join('') : `<tr><td colspan="9"><div class="empty-state"><div class="empty-state-icon">📭</div><div class="empty-state-title">Nessuna prenotazione</div><div class="empty-state-sub">Crea una nuova prenotazione con il pulsante in alto</div></div></td></tr>`;
}

function sortBookings(key) {
  if (bookingsSortKey === key) bookingsSortDir *= -1;
  else { bookingsSortKey = key; bookingsSortDir = 1; }
  renderBookingsTable();
}
...
function wizNext0() {
  if (!wizardData.eventId) {
    document.getElementById('err-wiz-event').textContent = 'Seleziona un evento per continuare';
    return;
  }
  wizardStep = 1;
  renderWizard();
...
function wizNext1() {
  let valid = true;
  const rules = [
    ['wiz-fname', 'err-wiz-fname', v => v.trim().length >= 2, 'Inserisci il nome (min 2 caratteri)'],
    ['wiz-lname', 'err-wiz-lname', v => v.trim().length >= 2, 'Inserisci il cognome (min 2 caratteri)'],
    ['wiz-email', 'err-wiz-email', v => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v), 'Email non valida'],
    ['wiz-phone', 'err-wiz-phone', v => v.trim().length >= 6, 'Numero di telefono non valido'],
    ['wiz-qty', 'err-wiz-qty', v => parseInt(v) > 0 && parseInt(v) <= 20, 'Posti tra 1 e 20'],
    ['wiz-bdate', 'err-wiz-bdate', v => v !== '', 'Inserisci la data di prenotazione'],
  ];
  ...
  if (evt && qty > (evt.capacity - evt.sold)) {
    document.getElementById('err-wiz-qty').textContent = `Max ${evt.capacity - evt.sold} posti disponibili`;
...
function renderWizardStep2() {
  ...
  let discountMsg = '';
  if (days >= 60) discountMsg = `🎉 Prenotazione con ${days} giorni di anticipo — sconto Early Bird 20%!`;
  else if (days >= 30) discountMsg = `✨ Prenotazione con ${days} giorni di anticipo — sconto 15%!`;
  else if (days >= 14) discountMsg = `👍 Prenotazione con ${days} giorni di anticipo — sconto 10%!`;
  else if (days >= 7) discountMsg = `Prenotazione con ${days} giorni di anticipo — sconto 5%`;
  else discountMsg = `Prenotazione ravvicinata (${days} giorni) — nessuno sconto`;
...
function renderReports() {
  ...
  document.getElementById('report-table').innerHTML = DB.bookings.map(b => {
    const evt = getEvent(b.eventId);
...
function renderCharts() {
  ...
  const typeCounts = {};
  DB.events.forEach(e => { typeCounts[e.type] = (typeCounts[e.type] || 0) + 1; });
  ...
  // BAR: tasso di riempimento
  const barLabels = DB.events.map(e => e.name.length > 18 ? e.name.slice(0, 18) + '…' : e.name);
  ...
  // LINE: andamento temporale prenotazioni per mese
  const monthMap = {};
  ...
  charts.line = new Chart(document.getElementById('chartLine'), {
    ...
    options: {
      ...
      scales: {
        ...
        y1: { position: 'right', grid: { drawOnChartArea: false }, ticks: { callback: v => '€' + v.toLocaleString('it-IT') } }
      },
